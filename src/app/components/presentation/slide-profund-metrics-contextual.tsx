...
export function SlideProfundMetricsContextual() {
  ...
  const visibilityChartData = MONTHS.map((month, index) => {
    ...
    VENDORS.forEach((vendor) => {
      if (visibleVendors.has(vendor.id)) {
        dataPoint[vendor.name] = editableData.visibility[vendor.id][index];
      }
    });
    return dataPoint;
  });
  ...
  const shareOfVoiceChartData = MONTHS.map((month, index) => {
    ...
    VENDORS.forEach((vendor) => {
      if (visibleVendors.has(vendor.id)) {
        dataPoint[vendor.name] = editableData.shareOfVoice[vendor.id][index];
      }
    });
    return dataPoint;
  });
  ...
  const citationChartData = MONTHS.map((month, index) => {
    ...
    VENDORS.forEach((vendor) => {
      if (visibleVendors.has(vendor.id)) {
        dataPoint[vendor.name] = editableData.citation[vendor.id][index];
      }
    });
    return dataPoint;
  });
              ...
              <ChartContainer title="Share of Voice %" height={200}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={shareOfVoiceChartData} margin={CHART_CONFIG.margin}>
                    ...
                    {VENDORS.map((vendor) =>
                      visibleVendors.has(vendor.id) ? (
                        <Line key={vendor.id} type="monotone" dataKey={vendor.name} stroke={vendor.color} {...CHART_CONFIG.line} />
                      ) : null
                    )}
                  </LineChart>
                </ResponsiveContainer>
